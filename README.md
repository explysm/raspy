...
📝 RAS Format Syntax
The format defines data lists using opening and closing delimiters.
...
# Assuming data.ras is in the current directory
data_file_path = os.path.join(os.path.dirname(__file__), 'data.ras')
...
# {'products': [['item_alpha', 'Alpha Product Description', 100, 'Warehouse A'], ...],
#  'status': [['product1', True, 45], ...]}

# 2. Accessing Specific Values with rasp_get()
The rasp_get() function now directly takes a file path and uses a three-part index for precise retrieval:
...
Running Tests
(Placeholder: You would define commands here to run tests)
